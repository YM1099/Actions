...
'''
ZodGame签到
https://github.com/y-tai/zodgame_checkin/blob/main/zodgame/zodgame.py
'''
import io
import re
import sys
...
def zodgame(cookie_string):
    ...
    if not len(driver.find_elements(By.XPATH, '//a[text()="用户名"]')) == 0:
        print("Login fails. Please check your cookie.")
        return 1
...
def send_msg():
    ...
    data = {"chat_id":TG_CHATID,"text":"【GitHub Actions】ZodGame签到失败"}
    requests.post(url=url, json=data)

if __name__ == "__main__":
    cookie_string = sys.argv[1]
    result = zodgame(cookie_string)
    if result == 1:
        send_msg()
